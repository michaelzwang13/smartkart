...
@meal_plan_compat_bp.route("/meal-plans/<int:plan_id>/notifications", methods=["GET"])
def get_pantry_change_notifications(plan_id):
    """Get pantry change notifications for a specific meal plan"""
    if "user_ID" not in session:
        return jsonify({"success": False, "message": "Not authenticated"})
    ...
    try:
        # Get unacknowledged notifications for this meal plan
        cursor.execute("""
            SELECT notification_id, change_type, affected_item_name, change_timestamp
            FROM pantry_change_notifications
            WHERE user_id = %s 
                AND meal_plan_session_id = %s
        ...
        for notification in notifications:
            formatted_notifications.append({
                ...
                "item_name": notification["affected_item_name"],
                "timestamp": notification["change_timestamp"].isoformat() if notification["change_timestamp"] else None
            })
...
@meal_plan_compat_bp.route("/meal-plans/<int:plan_id>/notifications/acknowledge", methods=["POST"])
def acknowledge_notifications(plan_id):
    """Mark pantry change notifications as acknowledged"""
    if "user_ID" not in session:
        return jsonify({"success": False, "message": "Not authenticated"})
    ...
    try:
        if notification_ids:
            ...
            cursor.execute(f"""
                UPDATE pantry_change_notifications 
                SET is_acknowledged = TRUE, acknowledged_at = CURRENT_TIMESTAMP
                WHERE user_id = %s 
                    AND meal_plan_session_id = %s
                    AND notification_id IN ({placeholders})
            """, [user_id, plan_id] + notification_ids)
        else:
            # Acknowledge all notifications for this meal plan
            cursor.execute("""
                UPDATE pantry_change_notifications 
                SET is_acknowledged = TRUE, acknowledged_at = CURRENT_TIMESTAMP
                WHERE user_id = %s AND meal_plan_session_id = %s AND is_acknowledged = FALSE
            """, (user_id, plan_id))
        ...
        return jsonify({
            "success": True,
            "message": f"Acknowledged {cursor.rowcount} notifications"
        })
        ...
        return jsonify({
            "success": False, 
            "message": f"Failed to acknowledge notifications: {str(e)}"
        })
    finally:
        cursor.close()
