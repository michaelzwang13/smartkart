...
def get_pantry_items():
    ...
    try:
        ...
        for item in all_items:
            if expiry_filter:
                if expiry_filter == 'expired' and item['expiry_status'] != 'expired':
                    continue
                elif expiry_filter == 'expiring_soon' and item['expiry_status'] != 'expiring_soon':
                    continue
                elif expiry_filter == 'fresh' and item['expiry_status'] not in ['fresh', 'no_expiry']:
                    continue
            items.append(item)
        ...
        return jsonify({'success': False, 'message': f'Failed to get pantry items: {str(e)}'})
    finally:
        cursor.close()
...
def add_pantry_item():
    ...
    try:
        ...
        print(f"DEBUG: Inserting pantry item - user_ID: {user_ID}, item_name: {item_name}, quantity: {quantity}")
        
        cursor.execute(query, (
            user_ID, item_name, quantity, unit, category, storage_type,
            expiration_date, 'manual', is_ai_predicted, notes
        ))
        ...
        db.rollback()
        return jsonify({'success': False, 'message': f'Failed to add pantry item: {str(e)}'})
    finally:
        cursor.close()
...
def predict_expiration_date(item_name, storage_type):
    ...
    try:
        ...
        if cached:
            ...
            from datetime import datetime, timedelta
            expiry_date = datetime.now().date() + timedelta(days=cached['predicted_days'])
            cursor.close()
...
def get_simple_prediction(item_name, storage_type):
    ...
    if any(word in item_lower for word in ['milk', 'dairy', 'yogurt', 'cheese']):
    ...
    elif any(word in item_lower for word in ['meat', 'chicken', 'beef', 'pork', 'fish']):
    ...
    elif any(word in item_lower for word in ['apple', 'banana', 'orange', 'fruit', 'vegetable']):
        ...
        # Default prediction
        base_days = 30 if storage_type == 'pantry' else 14 if storage_type == 'fridge' else 90
        return base_days
...
def update_pantry_item(item_id):
    ...
    try:
        ...
        cursor.execute(update_query, (
            item_name, quantity, unit, category, storage_type,
            expiration_date, is_ai_predicted, notes, item_id, user_id
        ))
        ...
        db.rollback()
        return jsonify({'success': False, 'message': f'Failed to update pantry item: {str(e)}'})
    finally:
        cursor.close()
...
def delete_pantry_item(item_id):
    ...
    try:
        ...
        # Delete the item
        delete_query = 'DELETE FROM pantry_items WHERE pantry_item_id = %s AND user_id = %s'
...
def test_pantry():
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) as count FROM pantry_items WHERE user_id = %s", (user_ID,))
        result = cursor.fetchone()
...
def generate_meal_plan():
    ...
    try:
        ...
        if not ingredients:
            ...
            ingredients = [f"{item['item_name']} ({item['quantity']} {item['unit']})" for item in pantry_items[:20]]  # Limit to 20 items
        ...
        cursor.execute(plan_query, (
            user_id, plan_name, start_date, end_date, days,
            ...
            for meal_type in ['breakfast', 'lunch', 'dinner']:
                if meal_type in day_data:
                    ...
                    cursor.execute(recipe_query, (
                        plan_id, meal_type, day_num, recipe_data.get('name', ''),
                        recipe_data.get('description', ''), recipe_data.get('prep_time', 0),
                        recipe_data.get('cook_time', 0), recipe_data.get('servings', 1),
                        recipe_data.get('cost', 0), recipe_data.get('difficulty', 'medium'),
                        recipe_data.get('calories', 0), recipe_data.get('instructions', ''),
                    ...
                    for ingredient in recipe_data.get('ingredients', []):
                        ...
                        cursor.execute(ingredient_query, (
                            recipe_id, ingredient.get('name', ''), ingredient.get('quantity', 1),
                            ingredient.get('unit', ''), ingredient.get('notes', ''), ingredient.get('cost', 0)
                        ))
        ...
        for step_data in meal_plan_data.get('batch_prep', []):
            ...
            cursor.execute(step_query, (
                plan_id, step_data.get('session_name', 'Prep Session'),
                step_data.get('order', 1), step_data.get('description', ''),
                step_data.get('time', 30), step_data.get('recipes', ''),
                step_data.get('equipment', ''), step_data.get('tips', '')
            ))
...
def generate_meal_plan_with_ai(days, ingredients, dietary_preference, budget, cooking_time):
    ...
    try:
        ...
        for item_data in consolidated.values():
            ...
            cursor.execute(shopping_query, (
                plan_id, item_data['name'], item_data['total_quantity'],
                item_data['unit'], item_data['total_cost'], category, False
            ))
