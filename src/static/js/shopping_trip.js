...
async function handleFormSubmit(event) {
    ...
    try {
        ...
        if (searchData.foods && searchData.foods.length > 0) {
            ...
            showNotification('⚠️ Product not found in database. Using generic data.', 'warning');
        } else {
            ...
            showNotification('⚠️ No product data available. Adding as manual entry.', 'warning');
        }
        ...
        if (addItemResponse.ok && resultData.items) {
            ...
            showNotification(`❌ Failed to add item: ${resultData.error || 'Unknown error'}`, 'error');
        }
...
function setupShoppingListEventListeners() {
    ...
    listSelect.addEventListener('change', function() {
        ...
        if (this.value) {
        startBtn.innerHTML = '<i class="fas fa-play" aria-hidden="true"></i> Start Shopping with List';
        showNotification('💡 List selected - click "Start Shopping with List" to begin', 'info');
        } else {
        startBtn.innerHTML = '<i class="fas fa-play" aria-hidden="true"></i> Start Shopping';
        }
    });
    }

    // Get cart ID from session
    if (CART_SESSION_EXISTS && "{{ cart_session.cart_ID if cart_session else '' }}") {
...
function displayAvailableLists(lists) {
    ...
    if (lists.length === 0) {
    showNotification('📝 No shopping lists found. Create one first!', 'warning');
    return;
    }
...
async function handleStartShopping() {
    ...
    if (listId) {
        ...
        if (cartResponse.ok) {
        showNotification(`✅ Shopping trip created with imported list!`, 'success');
        currentCartId = cartData.cart_id;
        setTimeout(() => {
            window.location.href = '/shopping-trip';
        }, 500);
        } else {
        showNotification(`❌ Failed to create trip: ${cartData.error}`, 'error');
        }
    } else {
...
async function loadShoppingListStatus(retryCount = 0) {
    ...
    if (retryCount < 3 && CART_SESSION_EXISTS) {
        console.log(`Retrying loadShoppingListStatus in ${(retryCount + 1) * 500}ms...`);
        setTimeout(() => {
    ...
    try {
    const response = await fetch(`/api/shopping-trip/list-status?cart_id=${currentCartId}`);
    const data = await response.json();
...
function displayShoppingListProgress(data) {
    ...
    const completedItems = data.items.filter(item => item.is_found || item.in_cart).length;
    ...
    return `
        <div class="shopping-list-item ${isFound ? 'found' : ''}" data-list-item-id="${item.list_item_id}">
        ...
        <div class="list-item-actions">
            ...
            <button class="btn-list-action btn-add-to-cart" 
                    onclick="quickAddToCart('${escapeHtml(item.name)}', ${item.quantity}, ${item.list_item_id})">
