...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Initialize advanced meal planning chatbot
  initializeChatbot();
...
// ============================================================================
// ADVANCED MEAL PLANNING CHATBOT FUNCTIONALITY
// ============================================================================
...
function initializeChatbot() {
  const advancedBtn = document.getElementById('advancedPlanningBtn');
  const modal = document.getElementById('advancedPlanningModal');
  const closeBtn = document.getElementById('closeChatbotModal');
  const chatInput = document.getElementById('chatInput');
  const sendBtn = document.getElementById('chatSendBtn');
  
  if (!advancedBtn || !modal) return;

  // Open chatbot modal
  advancedBtn.addEventListener('click', openChatbotModal);
...
function showAddPantryModal() {
  // For now, just show a message - could be expanded to a full modal later
...
function initializeDietaryPreferences() {
  ...
  // Copy the value from the main form if available
  const mainDietarySelect = document.getElementById('dietary_preference');
  if (mainDietarySelect) {
    dietarySelect.value = mainDietarySelect.value;
  }
  
  // Update main form when chatbot selection changes
  dietarySelect.addEventListener('change', (e) => {
    if (mainDietarySelect) {
      mainDietarySelect.value = e.target.value;
    }
  });
}
...
function handleMealPlanGeneration(mealPlanData) {
  ...
  if (confirmed) {
    ...
    // Populate the main meal plan form with the data
    populateMealPlanForm(mealPlanData);
    
    // Auto-generate the meal plan
    setTimeout(() => {
      generateMealPlan();
    }, 500);
  } else {
...
function populateMealPlanForm(data) {
  // Fill form fields with chat-derived data
  if (data.start_date) {
    const startDateInput = document.getElementById('start_date');
  ...
  if (data.dietary_preference) {
    const dietarySelect = document.getElementById('dietary_preference');
    if (dietarySelect) dietarySelect.value = data.dietary_preference;
  }
  ...
  if (data.cooking_time) {
    ...
    if (cookingTimeSelect) cookingTimeSelect.value = data.cooking_time;
  }
  
  // Store additional context for meal generation
  if (data.food_restrictions || data.ingredients) {
    window.chatbotMealPlanContext = {
      food_restrictions: data.food_restrictions || [],
      preferred_ingredients: data.ingredients || []
    };
  }
}
