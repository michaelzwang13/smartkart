...
function renderShoppingLists() {
    ...
    <div class="list-card" data-list-id="${list.id}" onclick="openListModal(${list.id}, event)">
        <div class="list-header">
        <div>
            <div class="list-header-top">
            <h3 class="list-name">${escapeHtml(list.name)}</h3>
            <div class="list-actions">
                <button class="btn-icon btn-edit" onclick="editList(${list.id})" title="Rename list">
                <i class="fas fa-edit"></i>
                </button>
                <button class="btn-icon btn-delete" onclick="deleteList(${list.id})" title="Delete list">
...
async function addItemToList(event, listId) {
    ...
    showNotification('Cannot add more items. Maximum 25 items per list.', 'error');
    return;
    }
...
function openListModal(listId, event) {
    ...
    if (event.target.closest('.list-actions') || event.target.closest('.btn-icon')) {
    ...
    <ul class="list-items">
        ...
        <li class="list-item">
            ...
            <input type="text" class="item-text-input ${item.is_completed ? 'completed' : ''}" 
                    value="${escapeHtml(item.name)}" 
                    onblur="updateItemNameInline(${list.id}, ${item.id}, this.value)" 
                    onkeypress="handleItemNameKeypress(event, ${list.id}, ${item.id})" 
                    title="Edit item name directly">
            <div class="item-controls">
            <div class="quantity-controls">
                <button class="qty-btn qty-decrease" onclick="changeQuantity(${list.id}, ${item.id}, -1)" title="Decrease quantity">
                <i class="fas fa-minus"></i>
                </button>
                <span class="item-quantity">${item.quantity}</span>
                <button class="qty-btn qty-increase" onclick="changeQuantity(${list.id}, ${item.id}, 1)" title="Increase quantity">
                <i class="fas fa-plus"></i>
                </button>
            </div>
            <button class="btn-delete-item" onclick="deleteItem(${list.id}, ${item.id})" title="Delete item">
...
document.getElementById('listModal').addEventListener('click', function(event) {
